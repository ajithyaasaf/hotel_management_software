...
// GET /api/expenses?from=&to=&category=&department=
router.get('/', requirePermission('expense.view'), async (req, res) => {
  try {
    const { from, to, category, department } = req.query;
    const where: any = {};
...
// GET /api/expenses/summary?from=&to=&department=
router.get('/summary', requirePermission('expense.view'), async (req, res) => {
  try {
    ...
    expenses.forEach(e => {
      ...
      byDepartment[e.department] = (byDepartment[e.department] || 0) + Number(e.amount);
    });
    res.json({ total: parseFloat(total.toFixed(2)), byCategory, byDepartment, count: expenses.length });
  } catch { res.status(500).json({ error: 'Failed to fetch expense summary' }); }
});

// POST /api/expenses
router.post('/', requirePermission('expense.manage'), async (req: AuthRequest, res) => {
  try {
...
// PUT /api/expenses/:id
router.put('/:id', requirePermission('expense.manage'), async (req: AuthRequest, res) => {
  try {
    const data = expenseSchema.parse(req.body);
    const { id } = req.params;
...
router.delete('/:id', requirePermission('expense.manage'), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const existing = await prisma.expense.findUnique({ where: { id: id as string } });
    if (!existing) { res.status(404).json({ error: 'Expense not found' }); return; }
